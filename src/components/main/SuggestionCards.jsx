...
export default function SuggestionCards() {
  return (
    <div className="w-full max-w-[720px] max-lg:max-w-[540px] flex flex-wrap items-center justify-center gap-2 max-sm:gap-2">
      {suggestions.map((item, index) => (
        <button
          key={index}
          className={`
            flex-1 min-w-[200px] max-w-[215px] h-[100px] p-3.5 rounded-[10px]
            ...
            ${index < 2
              ? "max-lg:flex-[1_1_calc(50%-6px)] max-lg:max-w-[calc(50%-6px)] max-sm:flex-[1_1_calc(50%-4px)] max-sm:max-w-[400pxs] max-sm:h-[90px]"
              : "max-lg:flex-[1_1_calc(50%-6px)] max-lg:max-w-[calc(50%-6px)] max-sm:flex-[1_1_100%] max-sm:max-w-[400px] max-sm:h-[90px]"
            }
          `}
        >
          ...
          <span className="text-[10px] max-sm:text-[9.5px] leading-[140%] text-white/90 overflow-hidden text-ellipsis line-clamp-2">
            {item.text}
          </span>
        </button>
      ))}
